...
function AppContent() {
  ...
  useEffect(() => {
    if (isLoud && soundType && !roadSafetyActive && emergencyNotificationsEnabled) {
      ...
      if (isGuest) guestAddNotification({ id: Date.now(), type: 'EMERGENCY', message: `Emergency: ${soundType}`, soundType, timestamp: new Date().toISOString(), read: false, volume });
  ...
  const handleLogout = async () => {
    try { await signOut(auth); showToast(t('loggedOut') || "Logged out", "success"); } 
    catch { showToast(t('logoutFailed') || "Logout failed", "error"); }
  };
  const handleGuestMode = () => { setIsGuest(true); setUser(null); showToast(t('guestModeActivated') || "Guest mode activated", "success"); };
  const handleSignOutGuest = () => { setIsGuest(false); showToast(t('guestSignedOut') || "Signed out from guest mode", "info"); };
  const handleThemeChange = (theme) => { setCurrentTheme(theme); localStorage.setItem('accessibility_theme', theme); };
  ...
  const guestAddRelative = (data) => {
    const entry = { id: Date.now(), name: data.name.trim(), phone: data.phone?.trim() || '', email: data.email?.trim() || '', relation: data.relation || '', notifyByWhatsApp: data.notifyByWhatsApp !== false, notifyBySMS: data.notifyBySMS || false, notifyByCall: data.notifyByCall || false, notifyByDesktop: data.notifyByDesktop !== false, autoSendWhatsApp: data.autoSendWhatsApp || false, createdAt: new Date().toISOString() };
    setGuestRelatives(prev => { const updated = [...prev, entry]; localStorage.setItem('neth_sawan_guest_relatives', JSON.stringify(updated)); return updated; });
    return entry;
  };
  const guestRemoveRelative = (id) => { setGuestRelatives(prev => { const updated = prev.filter(r => r.id !== id); localStorage.setItem('neth_sawan_guest_relatives', JSON.stringify(updated)); return updated; }); };
  const guestUpdateRelative = (id, updates) => { setGuestRelatives(prev => { const updated = prev.map(r => r.id === id ? { ...r, ...updates } : r); localStorage.setItem('neth_sawan_guest_relatives', JSON.stringify(updated)); return updated; }); };
  const guestAddNotification = (notification) => { setGuestNotifications(prev => { const updated = [notification, ...prev].slice(0, 50); localStorage.setItem('neth_sawan_guest_notifications', JSON.stringify(updated)); return updated; }); };
  const guestMarkAsRead = (id) => setGuestNotifications(prev => prev.map(n => n.id === id ? { ...n, read: true } : n));
  const guestClearNotifications = () => { setGuestNotifications([]); localStorage.setItem('neth_sawan_guest_notifications', JSON.stringify([])); };
  ...
  const handleFallEmergency = async () => {
    ...
    try {
      await addDoc(collection(db, "emergency_alerts"), { alertType: "AUTOMATIC_FALL_DETECTION", status: "CRITICAL", timestamp: serverTimestamp(), userId: user ? user.uid : "GUEST_USER", location: currentLocation ? `${currentLocation.lat}, ${currentLocation.lng}` : "Location Unavailable" });
  ...
  if (authLoading) return ( <div className="loading-screen"><div className="loading-spinner"></div><p>Neth-Sawan Loading...</p><div>🤟</div></div> );
  if (!user && !isGuest) return <LandingPage onGuestMode={handleGuestMode} />;
  ...
  return (
    <div className="app-wrapper" style={{ fontSize: `${currentFontSize}px` }}>
      ...
      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} isOpen={sidebarVisible} onClose={() => setSidebarVisible(false)} user={user} isGuest={isGuest} onLogout={isGuest ? handleSignOutGuest : handleLogout} />
      <div className={`content-area ${sidebarVisible && window.innerWidth > 1024 ? 'sidebar-open' : ''}`}>
        <Header isListening={isListening} lang={lang} setLang={setLang} showToast={showToast} user={user} isGuest={isGuest} roadSafetyActive={roadSafetyActive} setRoadSafetyActive={setRoadSafetyActive} emergencyNotificationsEnabled={emergencyNotificationsEnabled} onToggleEmergencyNotifications={toggleEmergencyNotifications} />
        <main className="main-content">
            ...
            <>
              ...
              <div className="captions-sign-row">
                ...
                <div className="sign-box">
                  <div className="section-header"><span className="section-icon">🤟</span><h3>Sign Language Translator</h3><span className="sign-badge-small">Live from captions</span></div>
                  <div className="sign-content"><SignLanguageBox transcript={transcript} /></div>
                </div>
              </div>
              {/* Sound Monitor + Road Safety */}
              <div className="dashboard-primary">
                <VisualAlert isLoud={isLoud && emergencyNotificationsEnabled} soundType={soundType} volume={volume} threshold={threshold} onThresholdChange={setThreshold} soundHistory={currentSoundHistory} />
                <RoadSafetyMonitor isActive={roadSafetyActive} onAlert={(alert) => { if(emergencyNotificationsEnabled) { showToast(alert.description,'error'); setFlashEmergency(true); setEmergencyData({ soundType: alert.name, message: alert.description, timestamp: new Date(), volume: alert.volume }); setTimeout(()=>setFlashEmergency(false),5000); if(isGuest) guestAddNotification({ id:Date.now(), type:'ROAD_SAFETY', message:alert.description, soundType:alert.name, timestamp:new Date().toISOString(), read:false }); } }} showToast={showToast} />
              </div>
              {/* Sound Visualizer + History */}
          ...
          {activeTab === 'emergency' && (
            <div className="emergency-sos-card">
              <div className="sos-header"><h2>🆘 {t('sosCenter')}</h2></div>
              <div className="sos-button-large" onClick={() => { if(emergencyNotificationsEnabled) { setFlashEmergency(true); setEmergencyData({ soundType:'SOS', message:'Manual SOS Triggered!', timestamp:new Date() }); setTimeout(()=>setFlashEmergency(false),8000); showToast('🚨 SOS Activated!','error'); if(navigator.vibrate) navigator.vibrate([500,200,500]); } }}><span className="sos-icon">🆘</span><span className="sos-text">{t('sos')}</span></div>
              <div className="emergency-numbers"><button className="emergency-btn police" onClick={()=>window.location.href='tel:119'}>👮 {t('police')} (119)</button><button className="emergency-btn ambulance" onClick={()=>window.location.href='tel:1990'}>🚑 {t('ambulance')} (1990)</button></div>
              <div className="emergency-instructions-box"><h4>⚠️ {t('emergencyInstructions')}</h4><ul><li>{t('redFlashing')}</li><li>{t('vibration')}</li><li>{t('contactsNotify')}</li><li>{t('liveLocation')}</li><li>{t('notificationToggle') || "🔕 Notification Toggle = Disable alerts when needed (top of screen)"}</li></ul></div>
            </div>
          )}
          {activeTab === 'roadmonitor' && (
            <div style={{ maxWidth:'900px', margin:'0 auto' }}>
              <div style={{ marginBottom:'20px', textAlign:'center' }}><h2 style={{ fontSize:'28px', marginBottom:'8px' }}>🛣️ {t('roadSafetyMonitor')}</h2><p style={{ color:'var(--text-secondary)' }}>{t('dedicatedRoadSafety') || "Full‑screen detection of horns, sirens, engines – with direction & distance hints"}</p></div>
              <RoadSafetyMonitor isActive={true} onAlert={(alert) => { if(emergencyNotificationsEnabled) { showToast(alert.description,'error'); setFlashEmergency(true); setEmergencyData({ soundType:alert.name, message:alert.description, timestamp:new Date(), volume:alert.volume }); setTimeout(()=>setFlashEmergency(false),5000); if(isGuest) guestAddNotification({ id:Date.now(), type:'ROAD_SAFETY', message:alert.description, soundType:alert.name, timestamp:new Date().toISOString(), read:false }); } }} showToast={showToast} />
            </div>
          )}
          {activeTab === 'settings' && <AccessibilitySettings onThemeChange={handleThemeChange} onFontSizeChange={handleFontSizeChange} currentTheme={currentTheme} currentFontSize={currentFontSize} />}
        </main>
      </div>
